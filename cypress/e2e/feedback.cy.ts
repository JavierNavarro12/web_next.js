...
describe('Feedback Tests', () => {
  ...
  it('should navigate to feedback page', () => {
    ...
    // The desktop footer should have a button in the "hidden md:flex" section
    cy.get('footer .hidden.md\\:flex button').contains('Feedback').should('exist').click({ force: true })
  ...
  it('should find form elements', () => {
    ...
    cy.scrollTo('bottom')
    cy.get('footer .hidden.md\\:flex button').contains('Feedback').click({ force: true })
    cy.wait(3000)
  ...
  it('should submit feedback successfully', () => {
    ...
    cy.get('textarea[name="feedback"]').filter(':visible').first().type('This is a test feedback message about AIFinder. Great website!')
